...
public class SingleNodePlanner {
   ...
   * The leftmostRef's join will be inverted if it is an outer/semi/cross join.
   */
  private PlanNode createSpatialJoinPlan(Analyzer analyzer, TableRef leftmostRef,
      List<Pair<TableRef, PlanNode>> refPlans, List<SubplanRef> subplanRefs)
      throws ImpalaException {

    LOG.trace("createSpatialJoinPlan: " + leftmostRef.getUniqueAlias());
    ...
    while (!remainingRefs.isEmpty()) {
      ...
      for (Pair<TableRef, PlanNode> entry: remainingRefs) {
        ...
        if (invertJoin) {
          ref.setJoinOp(ref.getJoinOp().invert());
          candidate = createSpatialJoinNode(analyzer, rhsPlan, root, ref, null);
        } else {
          candidate = createSpatialJoinNode(analyzer, root, rhsPlan, null, ref);
        }
        if (candidate == null) continue;
        LOG.trace("cardinality=" + Long.toString(candidate.getCardinality()));
  ...
  */
  ...
  private PlanNode createSpatialJoinNode(
    ...
    if (tblRef.getJoinOp().isOuterJoin()) {
    	candidates = analyzer.getEqJoinConjuncts(Lists.newArrayList(tblRef.getId()), Lists.newArrayList(innerRef.getId()));
	} else {
	    candidates = analyzer.getEqJoinConjuncts(Lists.newArrayList(tblRef.getId()), new ArrayList<TupleId>());
	}
