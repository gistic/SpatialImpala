...
public class SpatialJoinStmt extends StatementBase {
  ...
  public void analyze(Analyzer analyzer) throws AnalysisException {
    ...
    if (!tableName1_.isFullyQualified()) {
      tableName1_ = new TableName(analyzer.getDefaultDb(), tableName1_.getTbl());
    }
    table1 = analyzer.getTable(tableName1_, Privilege.SELECT);

    if (!tableName2_.isFullyQualified()) {
      tableName2_ = new TableName(analyzer.getDefaultDb(), tableName2_.getTbl());
    }
    ...
    if (!(columnNames1.contains(TAG) && columnNames1.contains(X1)
        && columnNames1.contains(Y1) && columnNames1.contains(X2)
        && columnNames1.contains(Y2)))
      throw new AnalysisException(TABLE_NOT_SPATIAL_ERROR_MSG
          + " : Table doesn't have the required columns.");

    List<String> columnNames2 = spatialTable2.getColumnNames();
    if (!(columnNames2.contains(TAG) && columnNames2.contains(X1)
        && columnNames2.contains(Y1) && columnNames2.contains(X2)
        && columnNames2.contains(Y2)))
      throw new AnalysisException(TABLE_NOT_SPATIAL_ERROR_MSG
          + " : Table doesn't have the required columns.");
    ...
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName1_.toString(), X1)), null));
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName1_.toString(), Y1)), null));
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName1_.toString(), X2)), null));
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName1_.toString(), Y2)), null));
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName2_.toString(), X1)), null));
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName2_.toString(), Y1)), null));
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName2_.toString(), X2)), null));
    items.add(new SelectListItem(new SlotRef(
        Path.createRawPath(tableName2_.toString(), Y2)), null));
  ...
  private Expr createWherePredicate(List<GlobalIndexRecord> globalIndexes) {
    ...
    Expr wherePredicate = new BinaryPredicate(BinaryPredicate.Operator.LE, x11, x22);
    wherePredicate = new CompoundPredicate(CompoundPredicate.Operator.AND, wherePredicate,
        new BinaryPredicate(BinaryPredicate.Operator.LE, x21, x12));

    wherePredicate = new CompoundPredicate(CompoundPredicate.Operator.AND, wherePredicate,
        new BinaryPredicate(BinaryPredicate.Operator.LE, y11, y22));

    wherePredicate = new CompoundPredicate(CompoundPredicate.Operator.AND, wherePredicate,
        new BinaryPredicate(BinaryPredicate.Operator.LE, y21, y12));
