...
public class GlobalIndex implements CatalogObject {
  ...
  private static String GLOBAL_INDEX_READ_EXCEPTION_MSG =
      "Couldn't parse Global Index file: ";
  ...
  public List<GlobalIndexRecord> getGIsIntersectRectangle(Rectangle rect) {
    ...
    for (GlobalIndexRecord gIRecord : globalIndexMap.values()) {
      if (gIRecord.getMBR().intersects(rect)) {
        LOG.info("GI record: " + gIRecord.getMBR() + " intersect with: " + rect);
        globalIndexes.add(gIRecord);
      } else {
        LOG.info("GI record: " + gIRecord.getMBR() + " does not intersect with: " + rect);
      }
    }
    return globalIndexes;
  }
  ...
  public List<GlobalIndexRecord> getGIsContainedInRectangle(Rectangle rect) {
    ...
    for (GlobalIndexRecord gIRecord : globalIndexMap.values()) {
      if (rect.contains(gIRecord.getMBR())) {
        LOG.info("GI record: " + gIRecord.getMBR() + "is fully contaied in: " + rect);
        globalIndexes.add(gIRecord);
      } else {
        LOG.info("GI record: " + gIRecord.getMBR() + " is not fully contaied in: " + rect);
      }
    }
    return globalIndexes;
  }
  ...
  public TGlobalIndex toThrift() {
    HashMap<String, TGlobalIndexRecord> tGlobalIndexMap =
        new HashMap<String, TGlobalIndexRecord>();
  ...
  private static HashMap<String, GlobalIndexRecord> loadGlobalIndex(
    ...
    while (scanner.hasNext()) {
      ...
      GlobalIndexRecord gIRecord = new GlobalIndexRecord(id, separatedRecord[7],
  ...
  public static GlobalIndex loadAndCreateGlobalIndex(String tableName,
      String globalIndexPath, String indexes) {
