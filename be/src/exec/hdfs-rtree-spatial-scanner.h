...
#ifndef IMPALA_EXEC_HDFS_RTREE_SPATIAL_SCANNER_H
...
// HdfsScanner implementation that understands rtree-formatted records.
// Uses SSE instructions, if available, for performance.
class HdfsRTreeSpatialScanner : public HdfsScanner {
 public:
  HdfsRTreeSpatialScanner(HdfsScanNode* scan_node, RuntimeState* state);
  virtual ~HdfsRTreeSpatialScanner();
